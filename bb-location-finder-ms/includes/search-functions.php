...
class BB_Location_Search {
    ...
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('SQL Query: ' . $sql);
    }
    ...
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Query returned ' . count($results) . ' results');
        if (count($results) > 0) {
            bb_location_debug_log('First result: ' . json_encode($results[0]));
        }
    }
...
public function ajax_location_search() {
    ...
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Location search AJAX request received');
    }
    
    $location = isset($_POST['location']) ? sanitize_text_field($_POST['location']) : '';
    $radius = isset($_POST['radius']) ? floatval($_POST['radius']) : 0;
    $unit = isset($_POST['unit']) ? sanitize_text_field($_POST['unit']) : 'km';
    
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Search parameters: ' . json_encode([
            'location' => $location,
            'radius' => $radius,
            'unit' => $unit
        ]));
    ...
    if (!$coordinates) {
        if (function_exists('bb_location_debug_log')) {
            bb_location_debug_log('Geocoding failed for: ' . $location);
        }
    ...
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Geocoded coordinates: ' . json_encode($coordinates));
    }
    ...
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Users found: ' . count($users));
    }
    ...
    // Debug logging
    if (function_exists('bb_location_debug_log')) {
        bb_location_debug_log('Sending response with ' . count($formatted_users) . ' users');
    }
